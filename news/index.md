...
1.  The remaining S3 methods will be un-exported in the next release:
    ...
    All of these have observed downstream calls directly. If the call is
    in a CRAN/Bioconductor package, I’ll be reaching out to help migrate
    onto the generic. This will probably take at least a year, so don’t
    expect this to induce a CRAN release *per se* until 2027 (although
...
1.  The following S3 methods were directly removed from the NAMESPACE:
    ...
    `cbind.integer64`, `ceiling.integer64`, `cummax.integer64`,
    `cummin.integer64`, `cumprod.integer64`, `cumsum.integer64`,
    `diff.integer64`, `duplicated.integer64`, `floor.integer64`,
    ...
    infeasible – R’s evaluation rules are simply too complex to warrant
    an expensive haystack search for the generic in the call stack each
    time a method is invoked. In some cases it is also not possible,
    period.
