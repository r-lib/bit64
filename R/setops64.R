...
#'   the \code{base} package functions are called.
...
union = function(x, y) {
  ...
  if (class_x != target_class) {
    ...
    if (
      ...
      && isFALSE(getOption("bit64.promoteInteger64ToCharacter", FALSE))
    )
      x = as.character(x)
    x = as(x, target_class)
  }
